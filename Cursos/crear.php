...
if ($_POST) {
    ...
    if ($Imagen != '') {
        $fecha = new DateTime();
        $nombreArchivo_imagen = $fecha->getTimestamp() . '_' . $Imagen;
        $tmp = $_FILES['Imagen']['tmp_name'];
        if (!is_dir('./Imagen/')) {
            mkdir('./Imagen/', 0755, true);
        }
        move_uploaded_file($tmp, './Imagen/' . $nombreArchivo_imagen);
    } else {
        $nombreArchivo_imagen = 'default.jpg';
    }

    try {
        $sql = "INSERT INTO cursos (IDUsuario, Nombre, Descripcion, Precio, Imagen, Estado) 
                VALUES (?, ?, ?, ?, ?, 'Pendiente')";
